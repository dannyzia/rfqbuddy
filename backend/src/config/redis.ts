...
const redisClient = process.env.NODE_ENV === 'test'
  ? {
      // Mock Redis client for tests - comprehensive mock for all Redis operations
      on: () => {},
      disconnect: async () => {},
      get: async () => null,
      set: async () => 'OK',
  ...
  tls: process.env.NODE_ENV === 'production' ? {} : { rejectUnauthorized: false }, // Enable TLS with verification in production, disable in non-production
