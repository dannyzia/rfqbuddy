...
describe('CommitteeDashboard', () => {
  ...
  it('renders committee dashboard with loading state', () => {
    ...
    expect(screen.getByText('Manage tender evaluation committees and track progress')).toBeInTheDocument();
  });
  ...
  it('displays committee statistics', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('1')).toBeInTheDocument(); // Active committees
      expect(screen.getByText('1')).toBeInTheDocument(); // Completed committees
    });
  });
  ...
  it('displays committee creation dates', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Created: 1\/[0-9]+\/[0-9]+/)).toBeInTheDocument();
  ...
  it('displays empty state when no committees found', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('You are not assigned to any tender committees yet')).toBeInTheDocument();
    });
  });
  ...
  it('has functional refresh button', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/committee/my-committees', expect.objectContaining({
